...
public class RecordWriter extends AbstractItemStreamItemWriter<String> implements ResourceAwareItemWriterItemStream<String>,
        InitializingBean {

    public static final boolean DEFAULT_TRANSACTIONAL = true;

    protected static final Log logger = LogFactory.getLog(RecordWriter.class);
    ...
    public RecordWriter() {
        this.setExecutionContextName(ClassUtils.getShortName(RecordWriter.class));
    }
    ...
    public void write(List<? extends String> items) throws Exception {
        ...
        boolean isFileControlRecord = false;
        int lineCount = 0;
        for (String item : items) {
            lines.append(lineAggregator.aggregate(item) + lineSeparator);
            lineCount++;
            if(item.substring(0, 1).equals("9")) {
                isFileControlRecord = true;
                break;
            }
        }
        try {
        ...
        if(isFileControlRecord) {
            chunkCount++;
            write(items.subList(lineCount - 1, items.size()));
        }
    }
    ...
    private File getFileToWrite() {
        File file;
        try {
            file = new File(resource.getFile(), "chunk" + (chunkCount + 1) + ".txt");
        }
        catch (IOException e) {
